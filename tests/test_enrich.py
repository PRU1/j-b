...
class ExpansionTests(unittest.TestCase):
    ...
    def test_semantically_unsupported_expansion_preserves_last_valid_draft(self):
        from scrubber import schema
        from scrubber.semantic import review_candidate, load_reviews
        original_reviews = review_candidate(self.project, FakeBackend([]), self.draft["candidates"][0],
            self.plan, self.context, emit=lambda _: None)
        bad = deepcopy(self.proposals[0])
        bad["sections"][0]["entries"][0]["bullets"][0]["text"] += " Led the team."
        ...
        backend = RejectingBackend([bad, *self.draft["candidates"]])
        with patch("scrubber.enrich.compile_tex", return_value=measured(.94)):
            result = self.expand(backend)
        self.assertEqual(result, fixture()[3])
        self.assertEqual(load_reviews(self.project, "faithful"), original_reviews)
        attempt = read_json(self.project / "expansion.json")["faithful"]["attempts"][0]
        self.assertFalse(attempt["accepted"])
        self.assertIn("Leadership not sourced", " ".join(attempt["errors"]))
        self.assertIn("Leadership not sourced", " ".join(backend.calls[1][1]["revision_feedback"]))
    ...
    def test_workflow_verifies_before_expansion_and_requires_new_claim_review(self):
        ...
        self.assertEqual(len(backend.semantic_calls), 9)
