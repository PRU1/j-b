...
def expand_project(project, backend, draft, plan, outline, context, emit=print, attempts=4):
    ...
    for i, (candidate, sketch) in enumerate(zip(draft["candidates"], outline["candidates"])):
        ...
        for attempt in range(attempts):
            ...
            if not errors:
                previous_reviews = load_reviews(project, name)
                try:
                    reviews = review_candidate(project, backend, proposed, plan, context, emit)
                    semantic = check_candidate(proposed, plan, context["master"], context["job"],
                                               sketch, semantic_reviews=reviews)
                    errors += semantic["errors"]
                    write_json(history / "semantic-review.json", {"version": 1, "reviews": reviews})
                finally:
                    # Proposed text is not the saved candidate until all checks accept it.
                    write_json(project / name / "semantic-review.json",
                               {"version": 1, "reviews": previous_reviews})
            ...
            if not errors:
                write_json(project / name / "semantic-review.json", {"version": 1, "reviews": reviews})
