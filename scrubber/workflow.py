...
from .semantic import load_reviews, review_candidate
...
def new_project(root, label, context):
    ...
    write_json(path / "state.json", {"version": 2, "dependencies": {}, "approvals": {}})
    return path
...
def repair_draft(backend, instruction, data):
    ...
    for candidate, sketch in zip(repaired["candidates"], data["outline"]["candidates"]):
        report = check_candidate(candidate, data["plan"], data["master"], data["job"], sketch,
                                 semantic_reviews=data.get("semantic_reviews", {}).get(candidate["name"]))
...
def review_and_repair(project, backend, draft, plan, outline, context, emit, max_repairs):
    """Semantic failures use the same bounded local repair path as mechanical errors."""
    attempt = 0
    while True:
        records = {c["name"]: review_candidate(project, backend, c, plan, context, emit)
                   for c in draft["candidates"]}
        errors = []
        for candidate, sketch in zip(draft["candidates"], outline["candidates"]):
            report = check_candidate(candidate, plan, context["master"], context["job"], sketch,
                                     semantic_reviews=records[candidate["name"]])
            errors.extend(f"{candidate['name']}: {e}" for e in report["errors"])
        if not errors:
            (project / "semantic-errors.json").unlink(missing_ok=True)
            return draft
        write_json(project / "semantic-errors.json", {"errors": errors})
        if max_repairs and attempt >= max_repairs:
            verify_project(project, emit=emit)
            raise ValueError("Semantic review failed validation: " + "; ".join(errors))
        attempt += 1
        emit(f"Semantic review: repairing automatically ({attempt}/{max_repairs or 'unlimited'}): " + "; ".join(errors))
        data = {**context, "plan": plan, "outline": outline, "previous_value": draft,
                "revision_feedback": errors, "semantic_reviews": records}
        draft = repair_draft(backend, DRAFT_PROMPT, data)
        schema.validate(draft, schema.DRAFT)
        write_json(project / "draft.json", draft)
        # Mechanical errors are repaired before sending a new version for semantic review.
        mechanical = draft_checks(draft, plan, context, outline)
        while mechanical:
            if max_repairs and attempt >= max_repairs:
                write_json(project / "semantic-errors.json", {"errors": mechanical})
                verify_project(project, emit=emit)
                raise ValueError("Semantic repair failed mechanical validation: " + "; ".join(mechanical))
            attempt += 1
            draft = repair_draft(backend, DRAFT_PROMPT, {**context, "plan": plan,
                "outline": outline, "previous_value": draft, "revision_feedback": mechanical})
            schema.validate(draft, schema.DRAFT)
            write_json(project / "draft.json", draft)
            mechanical = draft_checks(draft, plan, context, outline)
...
def run(project, backend, ask=input, emit=print, revise=None, lean=False,
    ...
    initial_draft_hash = digest(draft)
    draft = review_and_repair(project, backend, draft, plan, outline, context, emit, max_repairs)
    if digest(draft) != initial_draft_hash:
        verify_project(project, lean=lean, emit=emit)
    ...
    for candidate, sketch in zip(draft["candidates"], outline["candidates"]):
        ...
        report = check_candidate(candidate, plan, context["master"], context["job"], sketch,
                                 ...
                                 semantic_reviews=load_reviews(project, name),
                                 semantic_approvals=state.get("semantic_approvals", []))
        folder = project / name
        folder.mkdir(exist_ok=True)
        write_json(folder / "evidence.json", [{"path": p, **c} for p, c, _ in claims(candidate)])
        if report["review"]:
            emit(f"\n{name}: review factual support and any uncertain job mappings:")
            for item in report["review"]:
                emit(f"\n{item['path']}: {item['text']}")
                emit(item["reason"])
                for evidence in item["evidence"]:
                    emit(f"  {evidence['source']}: {evidence['quote']}")
            response = ask("Do the cited sources support these claims and any flagged job mappings? [yes / no]: ").strip().lower()
            if response in ("yes", "y"):
                state["approvals"][approval_key] = claim_approval(candidate, context)
                state.setdefault("semantic_approvals", []).extend(
                    item["semantic_approval"] for item in report["review"] if "semantic_approval" in item)
                write_json(project / "state.json", state)
            else:
...
def verify_project(project, lean=False, emit=print):
    ...
    for candidate, sketch in zip(draft["candidates"], outline["candidates"]):
        ...
        report = check_candidate(candidate, plan, context["master"], context["job"], sketch,
            ...
            semantic_reviews=load_reviews(project, candidate["name"]),
            semantic_approvals=state.get("semantic_approvals", []))
        ...
        (folder / "keywords.lean").write_text(lean_certificate(report, candidate, plan, context["job"], keyword_only=True), encoding="utf-8")
        if lean:
            if not shutil.which("lean"):
                ...
                with progress(f"Checking Lean certificate for {candidate['name']}"):
                    certificate = "constraints.lean" if report["status"] == "pass" else "keywords.lean"
                    result = subprocess.run(["lean", certificate], cwd=folder,
                                            capture_output=True, text=True, timeout=60)
                report["lean"] = {"passed": result.returncode == 0, "scope": certificate,
